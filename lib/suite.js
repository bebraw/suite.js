...
function generateTests(f, invariant, generator, amount) {
    ...
    for(var i = 0; i < amount; i++) {
        tests.push(generateTest(f, invariants, generator));
...
function generateTest(f, invariants, generator) {
    var params;
    var param;
    ...
    while(!isOk(f, params)) {
        ...
        for(var i = 0; params.length < invariants.maxLen; i++) {
            inv = invariant[i];

            if(funkit.isFunction(inv)) params.push(generator());
            else params.push(funkit.isDefined(inv)? inv: generator());
        }
    }
